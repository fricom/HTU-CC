...
- [부트캠프 한눈에 보기](#부트캠프-한눈에-보기)
- [1. 기본 세팅 — 환경 만들기](#1-기본-세팅--환경-만들기)
  - [1-1. 사전 준비](#1-1-사전-준비)
  - [1-2. 터미널 & 패키지 매니저](#1-2-터미널--패키지-매니저)
  - [1-3. 기본 도구 설치](#1-3-기본-도구-설치)
  - [1-4. alias 설정 & 첫 실행](#1-4-alias-설정--첫-실행)
  - [1-5. 첫 앱 빌드 + 깃 + 배포](#1-5-첫-앱-빌드--깃--배포)
  - [1-6. 환경 완성 (Cursor / tmux / Termius / Tailscale)](#1-6-환경-완성-cursor--tmux--termius--tailscale)
- [2. 아키텍처 이해 — 진짜 서비스로](#2-아키텍처-이해--진짜-서비스로)
- [3. CC 확장 — 내 능력 부여하기](#3-cc-확장--내-능력-부여하기)
- [4. 실전 도구 + 자동화 — 돌려놓고 퇴근하기](#4-실전-도구--자동화--돌려놓고-퇴근하기)
- [부록 A. 도구별 "왜 깔았는지" 요약 카드](#부록-a-도구별-왜-깔았는지-요약-카드)
- [부록 B. 앞으로 활용하기 좋은 시나리오](#부록-b-앞으로-활용하기-좋은-시나리오)
...
| 단계 | 내용 |
|---|---|
| 1 | [claude.ai](https://claude.ai)에 접속해 본인 이메일로 가입 |
...
> 💡 **무엇을 한 것?** Claude Code(CC)가 본인 계정의 API/사용량을 인식하도록 하는 단계. CC 첫 실행 시 브라우저로 자동 로그인 창이 떠서, 이 계정을 연결한다.
...
> - `work` → 작업 폴더로 빠르게 이동
>
> ⚠️ `--dangerously-skip-permissions`는 CC가 매번 "이 명령어 실행해도 될까요?"를 묻지 않게 한다. 빌더가 모든 명령어를 일일이 확인할 필요가 없게 되지만, **본인 PC에 한해 사용**할 것.
...
→ 브라우저가 열리며 로그인 요청 → 본인 계정으로 로그인 → 터미널로 자동 복귀.
...
**왜 필요한가?** 집 컴퓨터에 CC를 돌려놓고, 카페에서 스마트폰으로 진행 상황을 확인하고 싶을 때.

- 스마트폰/태블릿에서 내 컴퓨터 터미널 접속 (Mac/Windows 모두 지원)
- 앱스토어/플레이스토어에서 "Termius" 검색해 무료 설치
- 본인 PC의 SSH 정보 등록

> 💡 **활용 시나리오**: 집 데스크탑에 tmux로 CC 세션 띄워놓고 → 출근 → 회사에서 스마트폰 Termius로 접속 → CC가 어디까지 했는지 확인 → 지시 추가.

#### ✅ Tailscale — 어디서든 안전하게 연결

**왜 필요한가?** Termius로 외부에서 집 컴퓨터에 접속하려면 보안 연결이 필요하다. Tailscale은 그 보안 연결을 한 줄로 설정해주는 도구.

- [tailscale.com](https://tailscale.com) 가입 (무료 플랜으로 충분)
- 본인 컴퓨터와 스마트폰에 모두 설치 → 같은 계정으로 로그인
- 두 기기가 마치 같은 네트워크에 있는 것처럼 동작

> 💡 **이 모든 셋팅이 완료되면**: 집 컴퓨터에 CC를 돌려놓고, 카페에서 스마트폰으로 진행 상황을 확인하고 지시를 내릴 수 있다.
...
- **본인 한계 인식의 책임**: 모르는 건 모른다고 아는 것. 모르는 걸 아는 척하고 배포하면 사고가 남.
...
> ※ 이 세 가지를 조합하면: **퇴근할 때 CC에게 작업을 시키고, 다음 날 출근하면 결과가 나와 있는 환경**이 만들어진다.

### 4-2. wj-magic — 올인원 플러그인
...
> ℹ️ 위 패키지 경로는 부트캠프 시점 기준. 실제 패키지명이 다를 수 있으니 강사 안내를 따를 것.

### 4-5. 자동화 워크플로우 — 돌려놓고 퇴근하기
...
| **Termius** | 모바일 SSH 클라이언트 | 외부에서 폰으로 내 PC 접속 | 집/회사 PC 앞에서만 작업 가능 |
| **Tailscale** | 보안 메시 VPN | 어디서든 안전하게 내 PC 연결 | 외부 접속 보안 위험 |
...
| 외출 중 PC에서 돌아가는 작업 확인 | Termius + Tailscale로 폰에서 접속 |
...
> 패턴: **퇴근 전 시키고 → 자는 동안 CC가 일하고 → 출근하면 결과 확인**
...
3. **외출 중 진행 상황 체크**:
   ```
   Termius (스마트폰) → Tailscale 통해 집 PC 접속 → tmux attach → CC 진행 상황 확인
   ```
