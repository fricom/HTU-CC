...
> 🪟 **Windows 사용자 추가 준비**: 외부에서 접속하려면 메인 PC에 SSH 서버(OpenSSH)가 켜져 있어야 한다. 맥은 기본 제공되지만 윈도우는 별도 설치가 필요하다. 설치 방법은 아래 가이드를 따른다.
>
> 📄 **[OpenSSH 설치 가이드 (PDF) 다운로드](assets/openssh_install_guide.pdf)**
...
**왜 필요한가?** 지금까지 만든 메모장은 브라우저를 닫으면 데이터가 사라진다. Firestore를 연결하면 데이터가 구글 서버에 저장된다. 개발로 풀어야 할 것들을 무료로 서비스 해줌. 여기에 디비저장
