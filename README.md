...
부트캠프의 전체 그림은 **"바이브 코더가 한 명의 빌더가 되는 4단계"** 다.
...
> ℹ️ 이미 설치된 도구가 있다면 `which 도구이름` 으로 확인 가능. 부트캠프 표준(fnm 등)으로 통일 권장. 환경이 같아야 트러블슈팅이 쉽다.
...
> ℹ️ Firebase Spark 플랜은 완전 무료. 부트캠프 범위에서 과금 걱정 없음.
...
> ℹ️ 위 패키지 경로는 부트캠프 시점 기준. 실제 패키지명이 다를 수 있으니 공식 안내를 따를 것.
